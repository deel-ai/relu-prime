...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Pytorch relu experiment impact of different values for ReLU'(0) with best lr")
    parser.add_argument('--epochs', type=int, default=200, help='nb epochs')
    parser.add_argument('--nb_experiment', type=int, default=5, help='number of experiment independant run for each configuration')
    ...
    best_lrs = pd.read_csv(f'results/best_learning_rates/MNIST_v3times/batch_norm_{batch_norm}/sgd/best_lr.csv')
    relu_values = best_lrs.relu.values
    learning_rates =best_lrs.lr.values
    
    results_df = pd.DataFrame(columns=["run_id", "epoch", "train_loss", "train_accuracy", "test_loss", "test_accuracy", "relu", "lr"])
    for relu_value, lr in tqdm(zip(relu_values, learning_rates), desc='relu_value', leave=False):
