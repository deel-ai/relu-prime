...
def init():
    if args['network'] == 'vgg11':
        if args['batch_norm']:
            net = VGG('VGG11', batch_norm=True, relu_fn=lambda: ReLUAlpha(args['relu'])).to(device)
        else:
        ...
        if args['batch_norm']:
            net = resnet18(norm_layer=nn.BatchNorm2d, relu_fn=lambda: ReLUAlpha(args['relu'])).to(device)
        else:
            net = resnet18(norm_layer=nn.Identity, relu_fn=lambda: ReLUAlpha(args['relu'])).to(device)
    if device == 'cuda':
...
if __name__ == "__main__":
    ...
    parser.add_argument('--nb_experiment', type=int, default=3, help='number of experiment independant run for each configuration')
    ...
    print(f"Running {args['network']} with ReLU'(0)={args['relu']} for {n_epochs} epochs on CIFAR10")
